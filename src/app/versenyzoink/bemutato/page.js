          ...
          <section key={driver.id} className="h-screen w-full snap-start flex items-center justify-center px-6 md:px-20">
            <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
                <span className="text-4xl text-red-600 font-black text-8xl opacity-10 italic block mb-[-40px]">{driver.szam}</span>
                <h2 className="text-6xl md:text-8xl font-black italic uppercase mb-4">{driver.nev}</h2>
